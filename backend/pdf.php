...
$query = "SELECT c.nom, c.prenom, c.adresse, c.telephone, 
                 f.num_facture, f.date_creation, f.date_echeance, 
                 f.methode_paiement, fp.prix_total
          FROM facture f 
...
$query_produits = "SELECT p.nom, fp.quantite_total, fp.prix_total 
...
foreach ($produits as $produit) {
    ...
    $pdf->SetY($y_position);
    $pdf->SetX(120); 
    $pdf->Cell(24, $ligne_height, $produit['quantite_total'], 1, 0, 'C');  
    $pdf->Cell(24, $ligne_height, number_format($produit['prix_total'] / $produit['quantite_total'], 2) . 'dh', 1, 0, 'C');   
    $pdf->Cell(24, $ligne_height, number_format($produit['prix_total'], 2) . 'dh', 1, 1, 'C');  
}
...
$pdf->Cell(24, $ligne_height_total, number_format($facture['prix_total'], 2) . 'dh', 1, 1, 'C', 1);
